...
function make_random_pos_z() {
  return (
    Math.random() * -(Obstacle.SCENE_DEPTH / 2) +
    Obstacle.SCENE_DEPTH / 2 -
    1 / 2
  );
}
...
function make_random_pos_x() {
  return (
    Math.random() * (Obstacle.SCENE_WIDTH - 1 / 2) - Obstacle.SCENE_WIDTH / 2
  );
}
...
class Obstacle {
  ...
  move(move) {
    if (move.arrowUp) this.model.position.z += 0.2;
  ...
  static create_obstacles(
    scene,
    number_of_obstacles = this.DEFALT_OBSTACLE_NUMBER
  ) {
    const obstacles = [];
